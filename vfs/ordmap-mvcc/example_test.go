...
import (
	...
	"github.com/ncruces/go-sqlite3/vfs/ordmap-mvcc"
...
func TestExample(t *testing.T) {
	...
	db := assert(sql.Open("sqlite3", "file:/test.db?vfs=ordmapmvcc")).noErr(t)
	defer db.Close()
	...
	db2 := assert(sql.Open("sqlite3", "file:/test2.db?vfs=ordmapmvcc")).noErr(t)
	defer db2.Close()
